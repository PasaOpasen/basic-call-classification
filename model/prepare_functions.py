...
def split_by_speakers(line):
    """
    превращает строку диалога в набор пар текст-говорящий, при этом удаляя пробельные символы по краям
    и удаляя пустые записи
    """
    tmp = re.split(r'(\d\))', line)[1:]
    tmp = [t.strip().replace(')','') for t in tmp]
    
    result = []
    for i in range(0, len(tmp), 2):
        if len(tmp[i+1]):
            result.append((tmp[i+1], int(tmp[i])))
